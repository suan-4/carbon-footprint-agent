...
type WeeklyReportResponse = {
  ...
  trend: Array<{ day: string; carbonKg: number }>;
};
  ...
  async loadHomeData() {
    ...
    try {
      const [user, pointAccount, weeklyReport] = await Promise.all([
        ...
        request<WeeklyReportResponse>({ url: '/reports/weekly-overview' })
      ]);
      ...
      this.setData({
        ...
        campusNews: {
          quote: `${user?.nickname || '校园用户'} 本周已累计减碳 ${weeklyCarbonKg.toFixed(1)} kg，继续保持绿色节奏。`,
          author: '校园数据看板',
