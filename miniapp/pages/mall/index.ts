...
type ProductResponse = {
  id: number;
  name: string;
  category: 'physical' | 'virtual' | string;
  pointsCost: number;
  stock: number;
};
...
type RedeemResponse = {
  ...
  pointAccount: PointAccountResponse;
  product: ProductResponse;
  products: ProductResponse[];
};

type MallProductView = ProductResponse & {
  name: string;
  desc: string;
  hint: string;
  icon: string;
...
const PRODUCT_META: Record<number, { name: string; desc: string; hint: string; icon: string }> = {
  1: {
    name: '种子纸手账',
...
Page({
  ...
  normalizeProduct(product: ProductResponse, balance: number): MallProductView {
  ...
  async loadMallData() {
    ...
    try {
      const [pointAccount, products] = await Promise.all([
        request<PointAccountResponse>({ url: '/users/me/points' }),
        request<ProductResponse[]>({ url: '/products' })
      ]);

      const balance = pointAccount?.balance || 0;
      const normalizedProducts = (products || []).map((item) => this.normalizeProduct(item, balance));
  ...
  async redeemProduct(event: WechatMiniprogram.BaseEvent) {
    const { id, name, cost } = event.currentTarget.dataset as { id: number; name: string; cost: number };
    ...
    try {
      const result = await request<RedeemResponse>({
