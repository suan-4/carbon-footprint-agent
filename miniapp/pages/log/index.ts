...
Page({
  ...
  clearRewardToastTimer() {
    ...
    if (page.rewardToastTimer) {
      clearTimeout(page.rewardToastTimer);
      page.rewardToastTimer = null;
    }
  },

  async loadBehaviorCatalog() {
    try {
      const catalog = await request<BehaviorCatalogItem[]>({ url: '/behaviors/catalog' });
      const mapped = this.data.behaviorTypes.map((item: any) => {
  ...
  showRewardToast(payload: { behaviorName: string; pointsAwarded: number; carbonKg: number }) {
    const page = this as any;
    this.clearRewardToastTimer();
    ...
    page.rewardToastTimer = setTimeout(() => {
  ...
  hideRewardToast() {
    ...
    this.setData({
      rewardToast: {
        ...(this.data as any).rewardToast,
        visible: false
      }
    });
  },

  async selectBehavior(event: WechatMiniprogram.BaseEvent) {
    if (this.data.submitting) return;

    const { name, code } = event.currentTarget.dataset as { name: string; code: string };
    this.setData({ submitting: true });

    try {
      const result = await request<BehaviorSubmitResponse>({
        url: '/behaviors',
        method: 'POST',
        data: {
          behaviorCode: code,
